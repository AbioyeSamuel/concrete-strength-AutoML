...
# Import SymbolicRegressor from gplearn for genetic programming symbolic regression
...
if __name__ == "__main__":
    ...
    # Remove units from column names and extra text for consistency.
    # (Assuming the first 9 columns are features)
    input_variables = df.iloc[:, :9]
    input_variables.columns = [col.split(' (')[0].replace('Concrete ', '').lower() for col in input_variables.columns]

    # Plot Pearson correlation heatmap for input features
    ...
    # Compute Kendall's tau correlation heatmap
    ...
    # Assuming target column header is exactly 'CS\n(MPa)' in your file
    ...
    ###############################################
    # Step 3: Train a Genetic Programming Model (SymbolicRegressor)
    ###############################################
    # Create an instance of SymbolicRegressor.
    # Adjust parameters such as population_size, generations, etc., as needed.
    ...
    print("Training Symbolic Regressor using Genetic Programming...")
    start_time = time.time()
    symbolic_regressor.fit(X_train_scaled, y_train)
    training_time = time.time() - start_time
    print(f"Symbolic Regressor training complete in {training_time:.2f} seconds.")

    ###############################################
    # Step 4: Model Evaluation
    ###############################################
    y_pred = symbolic_regressor.predict(X_test_scaled)
    ...
    cv_scores = cross_val_score(symbolic_regressor, X_train_scaled, y_train, scoring="r2", cv=kf)
    ...
    plt.fill_between(range(1, n_splits+1), cv_scores.mean()-cv_scores.std(), cv_scores.mean()+cv_scores.std(),
                     color="red", alpha=0.2, label="Std Deviation Range")
    ...
    user_pred_strength = symbolic_regressor.predict(user_input_scaled)
    ...
    symbolic_regressor.fit(X_train_scaled, y_train)
    training_time = time.time() - start_time
    print(f"Training Time (Refit): {training_time:.2f} seconds")
    ...
    plt.scatter(sorted_indices, sorted_y_pred, color="blue", label="Predicted", alpha=0.7)
    plt.plot(sorted_indices, sorted_y_test, color="red", linestyle="-", linewidth=1)
    plt.plot(sorted_indices, sorted_y_pred, color="blue", linestyle="-", linewidth=1)
