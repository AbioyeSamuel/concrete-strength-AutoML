...
# Also import CatBoostRegressor so that TPOT uses the correct estimator for SHAP analysis later.
...
if __name__ == "__main__":
    ...
    # TPOTRegressor configured to only consider CatBoostRegressor with specified hyperparameters.
    tpot_config = {
    ...
    # For SHAP analysis, we refit an CatBoostRegressor using the best parameters found by TPOT
    # (Alternatively, you can use best_model if it is based on CatBoostRegressor)
    # Here we train a new model for clarity.
    model = CatBoostRegressor(n_estimators=200, learning_rate=0.1, max_depth=5, subsample=0.8, random_state=0)
    ...
    plt.figure(figsize=(8, 6))
    # This works only if the final step in the pipeline is indeed an CatBoostRegressor
    explainer = shap.TreeExplainer(best_model.named_steps['catboostregressor'])
